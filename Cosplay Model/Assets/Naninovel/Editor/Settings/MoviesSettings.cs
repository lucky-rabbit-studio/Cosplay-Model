...
namespace Naninovel
{
    public class MoviesSettings : ResourcefulSettings<MoviesConfiguration>
    {
        ...
        protected override string ResourcesSelectionTooltip => "Use `@movie %name%` in novel scripts to play a movie of the selected video clip.";
        protected override Dictionary<string, Action<SerializedProperty>> OverrideConfigurationDrawers => new Dictionary<string, Action<SerializedProperty>> {
            ["IntroMovieName"] = property => { if (Configuration.PlayIntroMovie) EditorResources.DrawPathPopup(property, ResourcesCategoryId, ResourcesPathPrefix); }
        };

...
namespace Naninovel
{
    public class BackgroundsSettings : ActorManagerSettings<BackgroundsConfiguration, IBackgroundActor, BackgroundMetadata>
    {
        ...
        protected override bool AllowMultipleResources => EditedMetadata?.Implementation != typeof(AnimatorBackground).FullName;
        ...
        private Type GetTypeConstraint ()
        {
            switch (EditedMetadata?.Implementation?.GetAfter("."))
            {
                ...
                case nameof(AnimatorBackground): return typeof(UnityEngine.Animator);
        ...
        private string GetTooltip ()
        {
            if (EditedActorId == BackgroundManager.MainActorId && AllowMultipleResources)
                return "Use `@back %name%` in novel scripts to show main background with the selected appearance.";
            else if (AllowMultipleResources)
                return $"Use `@back %name% id:{EditedActorId}` in novel scripts to show this background with the selected appearance.";
            return $"Use `@back id:{EditedActorId}` in novel scripts to show this background.";
        }

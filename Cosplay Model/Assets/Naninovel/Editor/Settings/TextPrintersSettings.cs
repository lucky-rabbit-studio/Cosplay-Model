    ...
    public class TextPrintersSettings : ActorManagerSettings<TextPrintersConfiguration, ITextPrinterActor, TextPrinterMetadata>
    {
        ...
        private string GetTooltip ()
        {
            if (EditedActorId == Configuration.DefaulPrinterId)
                return "This printer will be active by default: all the generic text and `@print` actions will use it to output the text. Use `@printer PrinterID` action to change active printer.";
            return $"Use `@printer {EditedActorId}` in novel scripts to set this printer active; all the consequent generic text and `@print` actions will then use it to output the text.";
        }

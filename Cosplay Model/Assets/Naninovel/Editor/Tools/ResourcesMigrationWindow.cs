...
namespace Naninovel
{
    public class ResourcesMigrationWindow : EditorWindow
    {
        ...
        {
            var position = new Rect(100, 100, 500, 180);
        ...
        private async void Upload ()
        {
            ...
            for (int i = 0; i < files.Length; i++)
            {
                ...
                if (file.Extension == ".txt") // Transform text files to google documents.
                {
                    googleFile.MimeType = Helpers.DocumentMimeType;
                    remoteFilePath = remoteFilePath.GetBeforeLast(".txt");
        ...
        private async Task FindAndDownloadFiles (string directoryPath)
        {
            ...
            for (int i = 0; i < files.Count; i++)
            {
                ...
                else
                {
                    ...
                    byte[] content = null;
                    if (file.MimeType == Helpers.DocumentMimeType) // Transform google documents to text files.
                    {
                        localFilePath += ".txt";

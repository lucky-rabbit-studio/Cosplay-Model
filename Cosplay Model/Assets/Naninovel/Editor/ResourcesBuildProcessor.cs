...
namespace Naninovel
{
    ...
    public class ResourcesBuildProcessor : IPreprocessBuildWithReport, IPostprocessBuildWithReport
    {
        ...
        public void OnPreprocessBuild (BuildReport report)
        {
            ...
            foreach (var record in records)
            {
                ...
                if (resourceAsset is UnityEngine.Video.VideoClip && report.summary.platform == BuildTarget.WebGL)
                    ProcessVideoResourceForWebGL(resourcePath, resourceAsset, provider);
                else ProcessResourceAsset(resourcePath, resourceAsset, provider);
            }
        ...
        private void ProcessResourceAsset (string path, Object asset, ProjectResourceProvider provider)
        {
            if (provider.ResourceExists<Object>(path)) return;
        ...
        /// </summary>
        private void ProcessVideoResourceForWebGL (string path, Object asset, ProjectResourceProvider provider)
        {
            var objPath = AssetDatabase.GetAssetPath(asset);

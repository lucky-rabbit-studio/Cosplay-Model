...
{
    /// <summary>
    /// Editor for <see cref="NovelActorMetadataMap{TMeta}"/>-derived properties.
    /// </summary>
    public class MetadataMapEditor
    {
        ...
        public class ElementModifiedArgs : EventArgs
        {
            ...
            /// </summary>
            public readonly NovelActorMetadata Metadata;
            /// <summary>
            /// Type of the modification done.
            /// </summary>
            public readonly ElementModificationType ModificationType;

            public ElementModifiedArgs (string actorId, NovelActorMetadata metadata, ElementModificationType modificationType)
            {
                ActorId = actorId;
                Metadata = metadata;
        ...
        public MetadataMapEditor (SerializedObject serializedObject, SerializedProperty mapProperty, Type metaType, string actorsLabel = "Actors", HashSet<string> lockedIds = null)
        {
            ...
            listHeaderLabel = new GUIContent($"{actorsLabel} List", "Double-click an element to edit actor metadata.");
        }
        ...
        {
            EditorGUI.LabelField(rect, listHeaderLabel);
        }
        ...
        private void HandleListElementAdded (ReorderableList list)
        {
            ...
            OnElementModified?.Invoke(new ElementModifiedArgs(string.Empty, metaProperty.GetGenericValue<NovelActorMetadata>(), ElementModificationType.Add));
        }

        private void HandleListElementRemoved (ReorderableList list)
        {
            var removedElementId = GetIdPropertyAt(list.index).stringValue;
            var removedElementMetadata = GetMetaPropertyAt(list.index).GetGenericValue<NovelActorMetadata>();

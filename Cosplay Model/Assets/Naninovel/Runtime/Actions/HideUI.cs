...
namespace Naninovel.Actions
{
    ...
    /// @hideUI Calendar
    /// </example>
    public class HideUI : NovelAction
    {
        ...
        public override Task ExecuteAsync ()
        {
            ...
            if (ui is null)
            {
                Debug.LogWarning($"Failed to execute {nameof(HideUI)} novel action: managed UI with prefab name `{UIPrefabName}` not found.");
        ...
        public override Task UndoAsync ()
        {
            ...
            if (ui is null)
                Debug.LogWarning($"Failed to undo {nameof(HideUI)} novel action: managed UI with prefab name `{undoData.UIPrefabName}` not found.");

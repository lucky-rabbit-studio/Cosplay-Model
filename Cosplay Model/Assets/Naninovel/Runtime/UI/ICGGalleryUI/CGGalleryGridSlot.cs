...
namespace Naninovel
{
    public class CGGalleryGridSlot : ScriptableGridSlot
    {
        ...
        {
            var cgTexture = default(Texture2D);

            if (textureLoader.IsLoaded(textureLocalPath))
                cgTexture = textureLoader.GetLoadedResourceOrNull(textureLocalPath);
            else
            {
        ...
        public async Task UnloadCGTextureAsync () => await textureLoader.UnloadAsync(textureLocalPath);

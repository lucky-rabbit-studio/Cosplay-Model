...
namespace Naninovel
{
    ...
    public class AudioManager : IStatefulService<SettingsStateMap>, IStatefulService<GameStateMap>
    {
        ...
        public async Task PreloadAudioAsync (string path)
        {
            await audioLoader.PreloadAsync(path);
        }

        public async Task UnloadAudioAsync (string path)
        {
            if (!audioLoader.IsLoaded(path)) return;

            audioController.StopClip(audioLoader.GetLoadedResourceOrNull(path));
            await audioLoader.UnloadAsync(path);
        }

        public async Task PreloadVoiceAsync (string path)
        {
            await voiceLoader.PreloadAsync(path);
        }

        public async Task UnloadVoiceAsync (string path)
        {
            if (!voiceLoader.IsLoaded(path)) return;

            audioController.StopClip(voiceLoader.GetLoadedResourceOrNull(path));
            await voiceLoader.UnloadAsync(path);
        }
        ...
        public bool IsVoicePlaying (string path)
        {
            ...
            var clipResource = voiceLoader.GetLoadedResourceOrNull(path);
            if (!clipResource.IsValid) return false;
            return audioController.GetTrack(clipResource)?.IsPlaying ?? false;
        }

        public async Task<bool> AudioExistsAsync (string path) => await audioLoader.ResourceExistsAsync(path);

        public async Task<bool> VoiceExistsAsync (string path) => await voiceLoader.ResourceExistsAsync(path);
        ...
        {
            if (!audioLoader.IsLoaded(path)) return;
            var clip = audioLoader.GetLoadedResourceOrNull(path);
            if (!restartIfPlaying && audioController.IsClipPlaying(clip)) return;
            audioController.PlayClip(clip, null, volume, false, sfxGroup);
        }

        public async Task PlayBgmAsync (string path, float volume = 1f, float fadeTime = 0f, bool loop = true)
        {
            ...
            var clip = await audioLoader.LoadAsync(path);
            if (!clip.IsValid)
            {
            ...
            if (fadeTime <= 0) audioController.PlayClip(clip, null, volume, loop, bgmGroup);
            else audioController.PlayClipAsync(clip, fadeTime, null, volume, loop, bgmGroup).WrapAsync();
        }

        public void StopBgm (string path, float fadeTime = 0f)
        {
            if (string.IsNullOrWhiteSpace(path)) return;
            bgmClips.RemoveAll(c => c.Path == path);

            if (!audioLoader.IsLoaded(path)) return;
            var clipResource = audioLoader.GetLoadedResourceOrNull(path);
            if (fadeTime <= 0) audioController.StopClip(clipResource);
            else audioController.StopClipAsync(clipResource, fadeTime).WrapAsync();
        }

        public void StopAllBgm (float fadeTime = 0f)
        {
            for (int i = 0; i < bgmClips.Count; i++)
            {
                var state = bgmClips[i];
                StopBgm(state.Path, fadeTime);
            }
        }
        ...
        public async Task PlaySfxAsync (string path, float volume = 1f, float fadeTime = 0f, bool loop = false)
        {
            ...
            if (fadeTime <= 0) audioController.PlayClip(clip, null, volume, loop, sfxGroup);
            else audioController.PlayClipAsync(clip, fadeTime, null, volume, loop, sfxGroup).WrapAsync();
        }

        public void StopSfx (string path, float fadeTime = 0f)
        {
            if (string.IsNullOrWhiteSpace(path)) return;
        ...
        public void StopVoice ()
        {
            ...
            var voiceResource = voiceLoader.GetLoadedResourceOrNull(voiceClip.Path);
            voiceClip = null;
            audioController.StopClip(voiceResource);
        }
